...
const VideoPlay = (props: VideoProps) => {
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://cricketwicket.biz/api/v1/related/video?video_id=${videoid}`,
        {
          method: 'GET',
          headers: myHeaders,
        },
      ...
      setCurrentVideo(responseData.length > 0 ? responseData[0] : null); // Set current video if available
    } catch (error) {
  ...
  const renderImageItem = ({item}) => {
    ...
    const thumbnailUrl = `https://img.youtube.com/vi/${videoId}/0.jpg`; // Fetching the default thumbnail
