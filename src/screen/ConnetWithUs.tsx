...
const ConnectWithUs = () => {
    ...
    const openUrl = async (appUrl, webUrl) => {
        try {
            const supported = await Linking.canOpenURL(appUrl);
            if (supported) {
                await Linking.openURL(appUrl);
            } else {
                await Linking.openURL(webUrl); // Fallback to browser if app isn't installed
            }
        } catch (err) {
            console.error('An error occurred while trying to open the URL:', err);
            Alert.alert('An error occurred', `Unable to open the URL: ${webUrl}`);
        }
    };
