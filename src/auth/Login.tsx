...
const handleUsernameChange = (text: string) => {
 ...
 const handleLogin = async () => {
  ...
  try {
    ...
    if (result.message === "OTP verified. User logged in successfully.") {
      ...
      if (result.errors && result.errors.password) {
        ...
        //   50
        // );
        Alert.alert(result.errors.password[0])
    ...
    //   50
    // );
       Alert.alert('Please check username & password.')
    setPassword("");
    setUsername("");
  }
  finally{
  ...
  //     iosClientId: '1095918266994-38l45ransbok24imtpvcuqf3akql522i.apps.googleusercontent.com',
  //     offlineAccess: true,
  //   });
  ...
  //     const { accessToken } = getToken; // Retrieve idToken from userInfo
  
  //     if (accessToken) {
  //       // Proceed with using the idToken
  //       console.log('idToken:', accessToken);
  //       const { id } = userInfo.user;
  //       await callGoogleLoginAPI(accessToken, id); 
  //     } else {
  //       // Handle scenario where idToken is null or undefined
  ...
  //       await AsyncStorage.setItem("loginToken", responseData.token); // Save token
  //       navigation.navigate('MyTab'); // Navigate to MyTab
  //     } else {
      ...
      <View style={styles.inputContainer}>
        <FloatingLabelInput
          ...
          customLabelStyles={{
            ...
            topFocused: -18, // Adjust top position when focused
            leftFocused: -10
